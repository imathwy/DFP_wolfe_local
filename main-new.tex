...
$\frac12 I\preceq\nabla^2 f(x)\preceq\frac32 I$ for all $x\in\R^2$.  We also
...
unbounded.  The accumulation points of the DFP sequence form a circle.  A
uniform separation bound allows us to interpolate the prescribed function
values and gradients.  We add smooth functions with pairwise disjoint supports
to a quadratic and keep the global Hessian bounds.  An affine change of
variables gives an identity-initialized example with problem-dependent Hessian
bounds.  An orthogonal direct sum extends the result to every dimension
$n\ge2$.
\end{abstract}
...
The classical DFP question and the main result are stated in
Section~\ref{sec:statement}.  Section~\ref{sec:strategy} gives the basic DFP
identities and an outline of the construction.  We develop the nonconvergent
sequence and its asymptotic behavior in
...
For $g_k=\nabla f(x_k)$, the classical DFP method written with an inverse
Hessian approximation is
\begin{align*}
...
\eqref{eq:open-question} fails.  The main theorem gives such an example.  Its
accepted steps even satisfy the standard strong Wolfe conditions.
...
$\widetilde f\in C^2(\R^2)$, $z_0\in\R^2$, positive step lengths $\alpha_k$,
and constants $0<\widetilde m\le \widetilde M<\infty$ such that
\[
\widetilde m I\preceq\nabla^2\widetilde f(z)
\preceq\widetilde M I\qquad(z\in\R^2).
\]
...
We first define sequences $(x_k,g_k,H_k)$ that satisfy the DFP
search-direction, secant, and update identities.  We then construct a globally
$C^2$ uniformly convex function whose values and gradients agree with these
data.  Separating the two steps makes the DFP recurrence explicit, but it also
creates an interpolation problem: the prescribed gradient differences must be
the secant vectors of a single objective.
...
basis.  Vector norms are Euclidean, and matrix norms are spectral norms.  With
this convention, five quantities describe the two-step sequence.  The table summarizes their
...
$\norm{L}<\lambda<1$, and fix a small Lipschitz bound $q_{\rm lip}>0$.
...
$[-a,a]\times\overline B_b$.  At $u=\pm a$, the modified $u$-component is the
identity.  The linear map $L$ sends $\overline B_b$ strictly inside itself.  By
...
$\zeta:[-a,a]\to\overline B_b$ with $\zeta(0)=0$ and
...
$z\mapsto\frac12\|z-C_k\|^2$, whose gradient at $x_k$ equals $g_k$.
...
\label{eq:C-half}
\end{align}
The two consecutive polar-angle increments within a cycle are respectively
...
\]
Every point $z$ in the $k$th support has
...
\]
If $z$ lies outside all supports, the three quantities are already zero.  If
$z$ lies in the $k$th support and tends to any $\zeta\in\Gamma$, then
$\operatorname{dist}(z,\Gamma)\le\norm{z-\zeta}$, and the displayed estimates
imply
\[
\frac{|\Psi(z)-\Psi(\zeta)-\nabla\Psi(\zeta)^T(z-\zeta)|}
     {\norm{z-\zeta}^2}\longrightarrow0,
\qquad
\frac{\norm{\nabla\Psi(z)-\nabla\Psi(\zeta)}}
     {\norm{z-\zeta}}\longrightarrow0,
\]
while $\nabla^2\Psi(z)\to0$.  The value, gradient, and Hessian in
...
Hessian bounds in Theorem~\ref{thm:main}.  At every iterate,
...
DFP preserves positive definiteness.  This completes the construction in
dimension two.  For $n>2$, define
\[
...
and inequalities are unchanged.  This proves Theorem~\ref{thm:main}.
...
\[
\widetilde f(z)=f(Lz),\quad z_k=L^{-1}x_k,\quad
\widetilde g_k=\nabla\widetilde f(z_k)=L^Tg_k,\quad
\widetilde H_k=L^{-1}H_kL^{-T}.
\]
Then $\widetilde H_0=I$, and the transformed search direction satisfies
\[
-\widetilde H_k\widetilde g_k
=L^{-1}(-H_kg_k).
\]
...
\[
\widetilde f(z_k+\alpha\widetilde d_k)
=f(x_k+\alpha d_k),\qquad
\nabla\widetilde f(z_k+\alpha\widetilde d_k)^T\widetilde d_k
=\nabla f(x_k+\alpha d_k)^Td_k.
\]
The Armijo and strong Wolfe curvature inequalities hold with the same
constants.  Also,
\[
\nabla^2\widetilde f(z)=L^T\nabla^2f(Lz)L,
\]
...
The matrices in \eqref{eq:alternating-secant-data} generate the prescribed
...
The proof of Theorem~\ref{thm:main} and
Corollary~\ref{cor:identity-initialization} is complete.  The experiments now
show the geometry of the construction, compare DFP and BFGS on the same finite
objective, and check the main asymptotic identities.  We keep two settings
separate.  In the first, the DFP steps are prescribed by the construction.  In
the second, a specific line-search routine chooses the steps.  The solid curves
in Figures~\ref{fig:orbit-comparison} and
...
display value $\epsilon_0=0.03$.  The recurrence is unchanged.  Panel~(a) follows the
two-step construction of
...
Panel~(b) uses the same display value, starting point, and initial
inverse-Hessian matrix, but replaces DFP by the classical inverse BFGS update.
The new update changes the search direction after the first secant pair.  The
DFP endpoints cannot be treated as a BFGS trajectory.  We instead
run BFGS on the corresponding finite $C^2$ interpolant, which can be evaluated
directly.  We use the strong Wolfe routine described below, initialized at
$\alpha=1$.
The line search selects the displayed path, and the gradient norm reaches
...
the geometry.  The next experiment uses one objective with
verified Hessian bounds; see Figure~\ref{fig:finite-gradient-comparison} and
Table~\ref{tab:finite-comparison}.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{Fig1.pdf}
\caption{Computed DFP and BFGS trajectories for $\epsilon_0=0.03$.
(a) The prescribed DFP sequence over $10^5$ two-step cycles.  Every point
satisfies the classical DFP recurrence, and all accepted steps satisfy the
standard strong Wolfe conditions.  We do not claim that a specific line-search
code selects the infinite sequence.  The dashed circle is the asymptotic
estimate $G_N\exp(-13\epsilon_N/3)$ centered at $C_N$, not a fit.  The open
square marks the final iterate.  (b) BFGS on the corresponding finite $C^2$
interpolant.  It uses the same starting point and initial inverse-Hessian
matrix, with a strong Wolfe line search initialized at $\alpha=1$.  The black
star marks its stationary point.  The larger value of $\epsilon_0$ is used
only to show the geometry.  Figure~\ref{fig:finite-gradient-comparison} gives
the same-objective comparison with verified Hessian bounds.}
\label{fig:orbit-comparison}
\end{figure}

Panel~(a) evaluates the recurrence used in the proof at a scale that shows its
limiting geometry.  We next compare the two methods on one finite objective
with verified Hessian bounds.  Both methods use the same line-search code.  We
take a finite sum of the interpolation functions with $8{,}004$ prescribed
endpoints and set $\epsilon_0=0.0025$.  The global Hessian bound is
\[
...
start from the same $x_0$ and $H_0$.  In both runs we call the
\texttt{scipy.optimize.line\_search} routine in SciPy~1.17.0 with
$(c_1,c_2)=(0.25,0.75)$, \texttt{amax=64}, and \texttt{maxiter=40}.
No previous step estimate is supplied, so the routine tests $\alpha=1$ first.
If that trial is rejected, it uses the same bracketing and zoom procedure in
both runs.  Here ``unit first'' does not mean that the step is fixed at one.
Only the
inverse quasi-Newton update differs between the two runs.
...
with $\epsilon_0=0.0025$.  Both methods use the same starting point, initial
inverse-Hessian matrix, strong Wolfe routine initialized at $\alpha=1$, and
$(c_1,c_2)=(0.25,0.75)$.  The DFP run stops after $5{,}000$ iterations.  BFGS
reaches the displayed tolerance after $32$ iterations.  The curves show the
computed histories; no fitted values are used.}
...
The DFP run in Figure~\ref{fig:orbit-comparison}(a) also checks the main
asymptotic and algebraic identities.  Table~\ref{tab:recurrence-verification}
compares the three leading coefficients with medians over the last $10{,}000$
cycles.  It also reports the largest algebraic residuals.  The DFP update is
...
not fitted to the orbit.

\begin{table}[!htbp]
\caption{Numerical checks of the asymptotic coefficients and algebraic
identities in the DFP sequence.  The first three entries are medians from the
final $10{,}000$ cycles; the two residuals are maxima over $2\times10^5$
steps.}
...
We also test how the initialization of the line search affects the finite
DFP run.  Let $\rho_k$ be the support radius and define
\[
...
$E_k>1$ are $10{,}141$ and $3{,}441$.  Three independent Wolfe implementations
initialized at $\alpha=1$ follow the prescribed sequence for $25{,}299$ and
$8{,}945$ iterations.  Every accepted unit step before the run leaves the
support also satisfies strong Wolfe.  If the first trial instead uses the
previous accepted step, two independent strong-Wolfe routines accept
$0.841685$ and $0.841667$ at the third iteration.  Both runs then leave the
corresponding support ball.  A fixed implementation can follow a very long
finite prefix, but the Wolfe inequalities do not select a unique infinite
sequence.
...
converge to a positive constant.  The standard strong Wolfe conditions alone
do not guarantee global convergence of DFP.  Between successive
cycle starts, the changes in the gradient norm are summable, but the total
eigenvector rotation is infinite.  Uniform separation of the iterates allows
